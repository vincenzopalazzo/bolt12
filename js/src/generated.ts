...
// ---------------------------------------------------------------------------
// payer_proof TLV type constants
// ---------------------------------------------------------------------------

export const PROOF_SIGNATURE = 241n;
export const PROOF_PREIMAGE = 1001n;
export const PROOF_OMITTED_TLVS = 1002n;
export const PROOF_MISSING_HASHES = 1003n;
export const PROOF_LEAF_HASHES = 1004n;
export const PROOF_NOTE = 1005n;

export const KNOWN_PAYER_PROOF_TYPES = new Set<bigint>([
  PROOF_SIGNATURE,
  PROOF_PREIMAGE,
  PROOF_OMITTED_TLVS,
...
/** Map from TLV type number to field name for payer_proof. */
export const PAYER_PROOF_TLV_NAMES: ReadonlyMap<bigint, string> = new Map([
  [2n, 'offer_chains'],
  [4n, 'offer_metadata'],
  [6n, 'offer_currency'],
  [8n, 'offer_amount'],
  ...
  [1002n, 'proof_omitted_tlvs'],
  [1003n, 'proof_missing_hashes'],
  [1004n, 'proof_leaf_hashes'],
  [1005n, 'proof_note'],
]);

/** Typed fields for a decoded payer_proof. */
export interface PayerProofFields {
  ...
  invoice_node_id?: Uint8Array;
  signature?: Uint8Array;
  proof_signature?: Uint8Array;
  proof_preimage?: Uint8Array;
  proof_omitted_tlvs?: bigint[];
  proof_missing_hashes?: Uint8Array[];
  proof_leaf_hashes?: Uint8Array[];
  proof_note?: string;
  /** Raw TLV records for advanced access. */
  records: TlvRecord[];
}
...
/**
 * Extract typed fields from payer_proof TLV records.
 */
export function extractPayerProofFields(records: TlvRecord[]): PayerProofFields {
  const fields: Partial<Omit<PayerProofFields, 'records'>> = {};
  ...
  for (const rec of records) {
    switch (rec.type) {
      ...
      case PROOF_SIGNATURE: {
        fields.proof_signature = rec.value.slice(0, 64);
        break;
      }
      case PROOF_PREIMAGE: {
        fields.proof_preimage = rec.value;
        break;
      }
      case PROOF_OMITTED_TLVS: {
        {
          ...
          fields.proof_omitted_tlvs = values;
        }
        break;
      }
      case PROOF_MISSING_HASHES: {
        {
          ...
          fields.proof_missing_hashes = hashes;
        }
        break;
      }
      case PROOF_LEAF_HASHES: {
        {
          ...
          fields.proof_leaf_hashes = hashes;
        }
        break;
      }
      case PROOF_NOTE: {
        fields.proof_note = utf8Decoder.decode(rec.value);
        break;
      }
    }
  }

  return { ...fields, records } as PayerProofFields;
}
