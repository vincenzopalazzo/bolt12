...
(function () {
  ...
  var EXAMPLES = {
    ex1: [
      '// Decode an offer - fields are available directly by name',
      ...
      'console.log("chains:", chains ?? "default (bitcoin)");',
      'console.log("total TLV records:", records.length);',
    ].join('\n'),
    ex2: [
      ...
      '',
      '// Step 1: Bech32 decode (works for lno, lnr, lni, lnp)',
      ...
      '',
      '// Step 3: Extract fields by type number',
      'const desc = records.find(r => r.type === 10n);',
      'console.log("description:", new TextDecoder().decode(desc.value));',
      '',
      '// Step 4: Compute offer_id (Merkle root)',
      'const root = bolt12.computeMerkleRoot(records);',
      'console.log("offer_id:", toHex(root));',
    ].join('\n'),
    ex3: [
      '// Try different offers - edit the string and press Run!',
      'const offers = [',
      ...
      '];',
      '',
      'for (const str of offers) {',
      '  try {',
      '    const { description, issuer_id, offer_id, amount, issuer } = bolt12.decodeOffer(str);',
      '    console.log("--- Valid Offer ---");',
      '    console.log("  description:", description);',
      '    console.log("  issuer:", issuer ?? "(none)");',
      '    console.log("  amount:", amount ? amount + " msat" : "(not set)");',
      '    console.log("  issuer_id:", issuer_id);',
      '    console.log("  offer_id:", toHex(offer_id));',
      '  } catch (e) {',
    ...
    ex5: [
      '// Verify a payer proof: decode, check signature, and inspect fields',
      '',
      '// First, create a proof so we have something to verify',
      'const result = bolt12.createPayerProof({',
      '  invoiceHex: "",',
      '  preimageHex: "6464646464646464646464646464646464646464646464646464646464646464",',
      '  payerSecretKeyHex: "2a2a2a2a2a2a2a2a2a2a2a2a2a2a2a2a2a2a2a2a2a2a2a2a2a2a2a2a2a2a2a2a",',
      ...
      '});',
      '',
      '// Now decode and verify the proof',
      'const decoded = bolt12.decodePayerProof(result.proofBech32);',
      'const proof = decoded.proof;',
      '',
      'console.log("=== Payer Proof Verification ===");',
      'console.log("");',
      '',
      '// Verify the invoice signature against the merkle root',
      'const verification = bolt12.verifyPayerProof(proof);',
      'console.log("invoice signature valid:", verification.valid);',
      'console.log("merkle root:", toHex(verification.merkleRoot));',
      '',
      '// Inspect included (disclosed) fields',
      'console.log("\\n--- Disclosed Fields ---");',
      'for (const rec of proof.includedRecords) {',
      '  console.log("  type", Number(rec.type) + ":", toHex(rec.value));',
      '}',
      '',
      '// Show omitted field markers',
      'console.log("\\n--- Omitted Markers ---");',
      'console.log("  positions:", proof.omittedTlvs.join(", "));',
      '',
      '// Payer identity and note',
      'console.log("\\n--- Payer Info ---");',
      'console.log("  payer_id:", toHex(proof.payerId));',
      'console.log("  note:", proof.payerNote ?? "(none)");',
      'console.log("  preimage:", proof.preimage ? toHex(proof.preimage) : "(not included)");',
    ].join('\n'),
  };
